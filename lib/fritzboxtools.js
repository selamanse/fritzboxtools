...
function fritzboxtools (uri) {
	...
	self.doRequest = function(options, dataRecieved){
		...
		util.log("Request for : "+ (options.ssl ? "https" : "http") + "://" + options.hostname + options.path);
		
		if (options.ssl === true){
			req = https.request(options, function(res){
				self.resHandling(res, dataRecieved);
			});
		}else{
	...
	self.resHandling = function(res, cb) {
	...
	self.getData = function(callback, cbinterval){
		...
		if(typeof cbinterval == "number" && cbinterval > 1000){
	...
	self.homeautoSwitch = function(deviceAin, switchCmd, callBack){
	
		util.log("requested SwitchCommand: " + switchCmd + " for AIN: " + deviceAin);
		
		var requestPath = "/webservices/homeautoswitch.lua?" + 
			(deviceAin ? "ain=" + encodeURI(deviceAin) : '') + 
			"&switchcmd=" + encodeURI(switchCmd) +
			"&sid=" + encodeURI(self._auth.SID);
		...
		self.doRequest(reqOptions, function(response){
			util.log(response);
